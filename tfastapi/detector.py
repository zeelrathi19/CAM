...
def load_config():
    ...
    bc_left = to_np(data.get("bc_left"))
    bc_right = to_np(data.get("bc_right"))
    bc_top = to_np(data.get("bc_top"))
    ab_bottom = to_np(data.get("ab_bottom"))
    bc_bottom = to_np(data.get("bc_bottom"))

    return A, B, C, ab_left, ab_right, ab_top, bc_left, bc_right, bc_top, ab_bottom, bc_bottom, restricted_areas
...
def check_point_in_restricted_areas(point, height, width, restricted_areas, ab_left, ab_right, bc_left, bc_right):
    """Check if a point falls in any of the restricted areas (quadrant-based)"""
    # Check both lines to determine which quadrant the point is in
...
def detect_and_classify(image_bytes):
    ...
    A, B, C, ab_left, ab_right, ab_top, bc_left, bc_right, bc_top, ab_bottom, bc_bottom, restricted_areas = load_config()
    ...
    # Areas arranged CLOCKWISE starting from bottom-left
            ...
            if contours:
                ...
                for x, y in all_contour_points:
                    if 0 <= int(x) < width and 0 <= int(y) < height:
                        ...
                        if check_point_in_restricted_areas(math_point, height, width, restricted_areas, ab_left, ab_right, bc_left, bc_right):
